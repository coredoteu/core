    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {items.map((item) => (
        <div key={item.code} className="border border-white/[0.06] p-5 flex flex-col gap-3">
          ...
          <span className="text-sm text-white/80 lowercase leading-snug">{item.name}</span>
          <p className="text-xs text-white/35 leading-relaxed lowercase">{item.desc}</p>
        </div>
      ))}
    </div>
...
function TimelineCard({
  ...
  return (
    <div className={`relative pl-12 md:pl-16 ${isLast ? "" : "pb-14 md:pb-20"}`}>
      <div className="absolute left-[13px] md:left-[17px] top-8 -translate-x-1/2 -translate-y-1/2 z-10">
        ...
        <span
          className={`relative block h-3 w-3 rounded-[50%] border ${statusTone === "live" ? "bg-white border-white" : "bg-[#0D0D0D] border-white/25"
            }`}
        />
      </div>
...
const v1ShampooActives: SpecItem[] = [
  { code: "v1.sh.01", name: "aloe vera juice", desc: "deep scalp hydration, soothes irritation and conditions at the root." },
  { code: "v1.sh.02", name: "sea kale extract", desc: "marine-derived fortification, strengthens the hair shaft from within." },
  { code: "v1.sh.03", name: "ginkgo biloba leaf extract", desc: "circulatory activator, stimulates scalp microcirculation for density." },
  { code: "v1.sh.04", name: "burdock root extract", desc: "sebum regulation, keeps scalp in clean equilibrium between washes." },
];

const v1ConditionerActives: SpecItem[] = [
  { code: "v1.co.01", name: "aloe vera juice", desc: "hydration delivery system, binds moisture into the cortex layer." },
  { code: "v1.co.02", name: "hydrolyzed wheat protein", desc: "structural repair, fills micro-damage along the hair shaft surface." },
  { code: "v1.co.03", name: "argan oil", desc: "lipid restoration, seals the cuticle for shine without grease." },
  { code: "v1.co.04", name: "sea kale extract", desc: "marine mineral complex, fortifies and thickens each strand." },
];
...
const v2ShampooActives: SpecItem[] = [
  { code: "v2.sh.01", name: "salix alba (willow bark) extract", desc: "natural exfoliant, clears buildup and supports scalp renewal." },
  { code: "v2.sh.02", name: "hydrolyzed pea protein", desc: "plant-based protein complex, reinforces strand structure without weight." },
  { code: "v2.sh.03", name: "rosmarinus officinalis extract", desc: "circulation-supporting botanical, sharpens the scalp environment." },
  { code: "v2.sh.04", name: "caffeine", desc: "follicle stimulant, engineered for density and vitality at the root." },
];

const v2ConditionerActives: SpecItem[] = [
  { code: "v2.co.01", name: "baobab protein", desc: "cuticle repair and reinforced strand strength." },
  { code: "v2.co.02", name: "plant squalane", desc: "moisture lock with a natural, weightless shine." },
  { code: "v2.co.03", name: "marshmallow root", desc: "instant slip, easier detangling on contact." },
  { code: "v2.co.04", name: "organic aloe vera", desc: "deep core hydration, carried through from the shampoo step." },
];

const packagingItems: SpecItem[] = [
  { code: "pkg.01", name: "outer box", desc: "matte black uncoated cardstock box, providing a minimal stealth unboxing experience." },
  { code: "pkg.02", name: "the reveal", desc: `inner flap featuring the core slogan: "refined to the core."` },
  { code: "pkg.03", name: "custom inlay", desc: "molded black paper pulp designed for asymmetric placement, holding one unit upright and the other inverted." },
  { code: "pkg.04", name: "insert card", desc: "heavyweight off-white a6 card outlining the technical philosophy behind the system." },
];

const researchItems: SpecItem[] = [
  { code: "rd.01", name: "scalp equilibrium serums", desc: "targeted concentrate for optimal ph and sebum balance between washes." },
  { code: "rd.02", name: "targeted leave-in actives", desc: "single-function leave-in treatments providing heat protection and density support." },
  { code: "rd.03", name: "custom dosage dispensers", desc: "precision-metered delivery hardware crafted for the entire system." },
];
        ...
        <div className="flex flex-col gap-10">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <SpecGroup label="unit 02 / conditioner v2" items={v2ConditionerActives} />
            </div>
          </div>
        </div>
      ),
    ...
    <section id="comparator" className="border-t border-white/10 py-20 md:py-32 scroll-mt-24">
      <div className="max-w-[1600px] mx-auto px-6 md:px-10">
        ...
        >
          ...
          <SpecGroup label={`unit 02 / conditioner / ${version}`} items={conditioner} />
        </motion.div>

        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-4">
          <UsageRoutine title="shampoo routine" steps={["massage", "cleanse", "rinse"]} />
          <UsageRoutine title="conditioner routine" steps={["apply", "wait", "rinse"]} />
        </div>
      </div>
    </section>
  );
