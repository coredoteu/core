  ...
  return (
    <div className="border border-hairline bg-[#0A0A0A]/80 backdrop-blur-md w-full">
      <div className="flex items-center justify-between gap-3 px-5 py-3.5 border-b border-hairline bg-white/[0.01]">
        <span className="min-w-0 flex-1 truncate font-mono text-[9px] tracking-[0.2em] text-white/40 lowercase">
          routine / {data.hairType} · {data.scalpCondition}
        </span>
        <span className="shrink-0 font-mono text-[9px] text-white/50 lowercase">{data.washesPerWeek}x / week</span>
      </div>
        ...
        {data.steps.map((step: any) => (
          <div key={step.order} className="flex items-start gap-4 px-5 py-3.5">
            ...
            <div className="min-w-0 flex flex-col gap-1">
              <span className="text-xs text-white/85 lowercase break-words">
                {step.action}{" "}
                <span className="text-white/35 font-mono text-[9px] ml-1">· {product(step.product)?.unit}</span>
              </span>
              <span className="text-[11px] text-white/45 lowercase leading-relaxed break-words">{step.detail}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {data.notes?.length > 0 && (
        <div className="px-5 py-3.5 border-t border-hairline bg-white/[0.015] flex flex-col gap-1.5">
          {data.notes.map((note: string, i: number) => (
            <p key={i} className="text-[10px] text-white/40 lowercase leading-relaxed break-words">· {note}</p>
          ))}
        </div>
      )}
    </div>
